...
def main():
    st.title('🎙️🤖Tranquilla Voice ChatBot🤖🎙️') # Set the title for the Streamlit web application
    ...
    if audio_bytes:
        ...
        if st.button('Get Response🎙️'):
            #Converting speech to text
            converted_text_openai = chatbot.speech_to_text_conversion(temp_audio_path)
            st.write("Transcription:",converted_text_openai)# Display the transcription on UI
            textmodel_response = chatbot.text_chat(converted_text_openai) #  # Generate response using text-based model
            audio_data = chatbot.text_to_speech_conversion(textmodel_response) # Convert the text response to audio format
